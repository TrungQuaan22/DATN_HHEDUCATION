...
import { Layout, BookOpen, Award, Sparkles, Home, X, ChevronLeft, User } from "lucide-react";
...
export default function StudentSidebar({
  ...
  const sidebarContent = (isMobile: boolean = false) => {
    ...
    return (
      <>
        {/* Brand Logo Header */}
        <div className={`py-6 border-b border-border-dark/50 flex flex-col gap-1.5 relative transition-all duration-300 ${
          collapsed ? "px-2" : "px-6"
        }`}>
          <div className="flex items-center overflow-hidden w-full relative h-10">
            ...
            {/* Expanded Full Logo */}
            <div className={`transition-all duration-300 transform origin-left flex items-center justify-between w-full ${
              collapsed ? "opacity-0 scale-75 translate-x-4 pointer-events-none" : "opacity-100 scale-100 translate-x-0"
            }`}>
              <Link
                href="/"
                onClick={handleLinkClick}
          ...
          <span className={`text-xs font-semibold text-brand-pink bg-brand-pink/10 px-2 py-0.5 rounded-full w-fit transition-all duration-300 ${
            collapsed ? "opacity-0 scale-75 pointer-events-none h-0 py-0 overflow-hidden mt-0" : "opacity-100 scale-100 mt-0"
          }`}>
        ...
        {/* Navigation Tabs */}
        <nav className={`flex-grow py-6 space-y-1 transition-all duration-300 ${collapsed ? "px-2" : "px-4"}`}>
          {navLinks.map((link) => {
        ...
        {/* Sidebar Footer */}
        <div className={`border-t border-border-dark/50 pt-4 pb-6 transition-all duration-300 space-y-3 ${
          collapsed ? "px-2" : "px-4"
        }`}>
          ...
          <div className="group relative pt-2">
            <div className={`flex items-center transition-all duration-300 ${collapsed ? "px-0 justify-center" : "gap-3 px-2"}`}>
              ...
              <div className={`truncate transition-all duration-300 ${
                collapsed ? "opacity-0 w-0 pointer-events-none" : "flex-grow opacity-100"
              }`}>
            ...
            {collapsed && (
              <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-2.5 py-1.5 bg-deep-black border border-border-dark text-cream text-xs font-black rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-[100] pointer-events-none select-none">
                {user?.fullName || "Nguyễn Văn A"} ({getRoleLabel(user?.role)})
                <div className="absolute right-full top-1/2 -translate-y-1/2 border-y-4 border-y-transparent border-r-4 border-r-deep-black mr-[-1px]"></div>
              </div>
            )}
          </div>
  ...
  return (
    <>
      {/* Sidebar Desktop */}
      <aside className={`hidden md:flex flex-col fixed left-0 top-0 h-full bg-deep-black border-r border-border-dark z-30 transition-all duration-300 ${
