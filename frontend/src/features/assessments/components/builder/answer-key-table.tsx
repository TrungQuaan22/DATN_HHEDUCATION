...
export function AnswerKeyTable({
  ...
  const [expandedExplanationId, setExpandedExplanationId] = useState<
    string | null
  >(null);
      ...
      <table className="w-full min-w-[920px] table-fixed text-left border-collapse text-xs">
        ...
        <thead>
          <tr className="border-b border-admin-border/60 text-admin-muted font-bold text-xs uppercase tracking-wider sticky top-0 bg-admin-surface-low z-10">
            ...
            <th className="px-2 pb-2 text-center">Hướng dẫn</th>
            <th className="px-2 pb-2"></th>
          </tr>
        </thead>
        <tbody className="divide-y divide-admin-border/40">
          {items.map((item) => {
            const isExplanationExpanded = expandedExplanationId === item.id;
            ...
            return (
              <React.Fragment key={item.id}>
                ...
                >
                    ...
                    {item.itemType === "mcq" && (
                      <div className="flex flex-wrap gap-1.5 items-center">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const nextMode =
                              item.mode === "multiple" ? "single" : "multiple";
                            handleItemFieldChange(item.id, {
                              mode: nextMode,
                              correctOptions:
                                nextMode === "single" &&
                  ...
                  >
                    <button
                      ...
                      aria-controls={`exam-explanation-${item.id}`}
                      onClick={() => {
                        onSelectItemId(item.id);
                        setExpandedExplanationId(
                          isExplanationExpanded ? null : item.id,
                        );
                      }}
                      className={`inline-flex h-8 items-center gap-1.5 rounded border px-2 text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-admin-pink/50 ${
                        hasExplanation
                          ? "border-admin-pink/35 bg-admin-pink/10 text-admin-pink"
                          : "border-admin-border bg-admin-deep text-admin-muted hover:border-admin-pink/50 hover:text-admin-cream"
                      }`}
                    >
                      {hasExplanation ? (
                      ...
                      {hasExplanation ? "Đã có" : "Thêm"}
                      <ChevronDown
                        size={12}
                        className={`transition-transform ${isExplanationExpanded ? "rotate-180" : ""}`}
                      />
                    </button>
                  </td>
                ...
                {isExplanationExpanded && (
                  <tr
                    id={`exam-explanation-${item.id}`}
                    className="bg-admin-bg/70"
                  >
                    <td colSpan={7} className="px-4 pb-4 pt-3">
                      <div className="mx-auto max-w-3xl space-y-2">
                        <div className="flex flex-wrap items-baseline justify-between gap-2">
                          <label
                            htmlFor={`exam-explanation-input-${item.id}`}
                            className="text-xs font-bold text-admin-cream"
                          >
                            Hướng dẫn và lời giải cho câu {item.questionNumber}
                          </label>
                          <span className="text-xs text-admin-muted">
                            Nội dung này chỉ hiển thị cho học sinh sau khi bài
                            làm được chấm xong.
                          </span>
                        </div>
                        <textarea
                          id={`exam-explanation-input-${item.id}`}
                          rows={4}
                          ...
                          className="w-full resize-y rounded border border-admin-border bg-admin-deep px-3 py-2.5 text-sm leading-6 text-admin-cream outline-none placeholder:text-admin-muted focus:border-admin-pink focus:ring-1 focus:ring-admin-pink/30"
                        />
                      </div>
                    </td>
                  </tr>
