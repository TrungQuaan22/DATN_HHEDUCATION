...
export const requireAuth = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (error instanceof TokenExpiredError) {
      return next(new AppError(401, ERROR_CODE.ACCESS_TOKEN_EXPIRED, ERROR_MESSAGE.ACCESS_TOKEN_EXPIRED));
    }

    return next(new AppError(401, ERROR_CODE.INVALID_ACCESS_TOKEN, ERROR_MESSAGE.INVALID_ACCESS_TOKEN));
  }
  ...
  if (session.user.status === UserStatus.pending_verification) {
    return next(new AppError(403, ERROR_CODE.ACCOUNT_NOT_VERIFIED, ERROR_MESSAGE.ACCOUNT_NOT_VERIFIED));
  }
